...
santa_pos = Coords.new(0,0)
houses = {}
houses[santa_pos.h] = 1

santa2_pos = Coords.new(0,0)
robo_pos = Coords.new(0,0)
houses_wrobo = {}
houses_wrobo[santa2_pos.h] = 2
...
input[0].chars { |c|
	...
	when '^'
		santa_pos.decy
		if santas_turn; santa2_pos.decy; else robo_pos.decy end
	when 'v'
		santa_pos.incy
		if santas_turn; santa2_pos.incy; else robo_pos.incy end
	when '<'
		santa_pos.decx
		if santas_turn; santa2_pos.decx; else robo_pos.decx end
	when '>'
		santa_pos.incx
		if santas_turn; santa2_pos.incx; else robo_pos.incx end
	else
		next
	end

	if houses.key?(santa_pos.h)
		houses[santa_pos.h] += 1
	else
		houses[santa_pos.h] = 1
	end

	delivery_pos = santas_turn ? santa2_pos : robo_pos
	if houses_wrobo.key?(delivery_pos.h)
		houses_wrobo[delivery_pos.h] += 1
	else
		houses_wrobo[delivery_pos.h] = 1
	end
...
# p houses
print 'santa: ', houses.length, "\n"
print "santa&robo: #{houses_wrobo.length}\n"
