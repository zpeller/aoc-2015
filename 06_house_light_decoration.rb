...
input = (ARGV.empty? ? DATA : ARGF).each_line.map { |l|
	  todo = l.scan(/(turn on|turn off|toggle)/).freeze
	  nums = l.scan(/-?\d+/).map(&:to_i).freeze
	  c1 = Coords.new(nums[0], nums[1])
	  c2 = Coords.new(nums[2], nums[3])
	  [todo[0][0], c1, c2]
}.freeze

lights_1 = Array.new(1000){ [0]*1000 }
lights_2 = Array.new(1000){ [0]*1000 }

input.each {|todo, c1, c2|
#	print todo, c1, c2, "\n"
	(c1.x..c2.x).each { |x|
		(c1.y..c2.y).each { |y|
#			print("x: #{x} y: #{y}\n")
			...
			when 'turn off'
				lights_1[x][y] = 0
				lights_2[x][y] -= 1
				if lights_2[x][y] <0; lights_2[x][y]=0; end
			when 'toggle'
				lights_1[x][y] = 1-lights_1[x][y]
				lights_2[x][y] += 2
			else
				abort("Invalid instruction: #{todo}\n")
			end
		}
	}
}

lights_on = lights_1.map{|x| x.inject(:+)}.inject(:+)
print("Problem 1 lights on: #{lights_on}\n")

total_brightness_2 = lights_2.map{|x| x.inject(:+)}.inject(:+)
print("Total brightness2: #{total_brightness_2}\n")
