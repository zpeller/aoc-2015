...
$scan_re2 = Regexp.new('^.*('+scan_patterns.uniq.sort_by(&:length).reverse.join('|')+')$')
print scan_patterns, "\n", $scan_re, "\n"
print scan_patterns, "\n", $scan_re2, "\n"
...
def find_and_replace_last_pattern(molecule)
	last_pattern_pos = molecule.rindex($scan_re)
	last_pattern = molecule[last_pattern_pos..-1].scan($scan_re)
	last_pattern = last_pattern[0].join
#	print last_pattern,': ', last_pattern_pos, "\n"
	new_molecule = molecule[0,last_pattern_pos] + $rev_replacements[last_pattern] + molecule[last_pattern_pos+last_pattern.length..-1]
end
...
steps = 0
(1..300).each { |n|
	test_molecule = find_and_replace_last_pattern(test_molecule)
...
# This solution is pure luck: find from right succeeds, but there's no guarantee
print("Problem 2 result: #{steps}\n")
