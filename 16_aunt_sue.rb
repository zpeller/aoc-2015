...
facts_input.each_line { |l|
	tokens = l.scan(/^(.*): (.*)$/)[0]
	facts[tokens[0]] = tokens[1].to_i
}
print facts, "\n"
...
input.each { |l|
	tokens = l.strip.scan(/^Sue (.*): (.*): (.*), (.*): (.*), (.*): (.*)$/)[0]
	all_good = true
	[1, 3, 5].each { |n|
		token = tokens[n]
		value = tokens[n+1].to_i
		if facts[token] != value
			all_good = false
			break
		end
	}
	if all_good 
		print("Problem 1. good Sue: #{l}\n")
		break
	end
}
...
input.each { |l|
	...
	[1, 3, 5].each { |n|
		...
		elsif token == 'pomeranians' or token == 'goldfish'
			if facts[token] <= value
				all_good = false
				break
			end
