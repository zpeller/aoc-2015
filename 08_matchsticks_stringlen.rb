...
input = (ARGV.empty? ? DATA : ARGF).each_line.map { |l|
	sl = l.strip
	unescaped_line = eval("x=#{sl}")
	[sl.length, unescaped_line.length, sl.inspect.length]
}.freeze

orig_len = input.reduce(0) { |sum, x| sum + x[0] }
string_len = input.reduce(0) { |sum, x| sum + x[1] }
insp_len = input.reduce(0) { |sum, x| sum + x[2] }

print("Orig len: #{orig_len} string_len: #{string_len} insp_len: #{insp_len} diff1: #{orig_len-string_len} diff2: #{insp_len-orig_len}\n")
